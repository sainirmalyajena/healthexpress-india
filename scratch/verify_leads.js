...
function get10Digit(phone) {
    if (!phone) return '';
    const digits = phone.replace(/^p:/i, '').replace(/\D/g, '');
...
async function main() {
    ...
    for (const l of existingLeads) {
        ...
        if (last10) existingPhones.set(last10, l);
    }
    ...
    for (const row of parsed.data) {
        const last10 = get10Digit(row.phone_number);
        if (existingPhones.has(last10)) {
            alreadyInSupabase++;
        } else {
            notInSupabase++;
            missingLeads.push({ name: row.full_name, phone: row.phone_number, city: row.city });
        }
    }

    console.log("-----------------------------------------");
    console.log(`Total CSV rows: ${parsed.data.length}`);
    console.log(`Already in Supabase CRM: ${alreadyInSupabase}`);
    console.log(`Not in Supabase CRM: ${notInSupabase}`);
